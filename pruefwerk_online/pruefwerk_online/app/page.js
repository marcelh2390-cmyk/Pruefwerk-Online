...
export default function Home() {
  ...
  function saveEmployee(e, id){
    e.preventDefault(); const f = new FormData(e.currentTarget)
    setEmployees(v => v.map(emp => emp.id===id ? {
      ...emp, name:f.get('name'), role:f.get('role'), status:f.get('status'), weeklyHours:+f.get('weeklyHours')||40
    } : emp))
    setEditingEmployeeId(null)
  }
  function deactivateEmployee(id){
    setEmployees(v => v.map(emp => emp.id===id ? {...emp,status:'Inaktiv'} : emp))
    setEditingEmployeeId(null)
  }
